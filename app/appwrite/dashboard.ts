...
export const getTripsByTravelStyle = async () => {
    ...
    const travelStyleCounts = trips.documents.reduce(
        (acc: { [key: string]: number }, trip: Document) => {
            // Appwrite documents store the trip JSON under `tripDetail` (singular).
            // Keep backward compatibility with `tripDetails` (plural) if the schema differs.
            const rawTripDetail = (trip.tripDetail ?? trip.tripDetails) as string | undefined;
            const tripDetail = rawTripDetail ? parseTripData(rawTripDetail) : null;
